...
st.title("🚀 高度経営シミュレーター (視認性完全修正版)")
st.caption("M4 MacBook Air 最適化 / あらゆる環境でストライプ表示を強制 / 単位：千円")
...
with col_input2:
    ...
    df_actions = st.data_editor(default_actions, num_rows="dynamic", use_container_width=True,
...
def display_styled_table(df):
    ...
    # PandasのStylerを使用してHTMLを生成
    # 背景色は rgba(128,128,128,0.1) を使用することでダーク/ライトどちらでも文字を隠さずストライプにする
    styled_df = df.style.format(format_dict).set_table_styles([
        {'selector': 'tr:nth-child(even)', 'props': [('background-color', 'rgba(128, 128, 128, 0.15)')]},
        {'selector': 'th', 'props': [('background-color', 'rgba(128, 128, 128, 0.2)'), ('color', 'inherit')]},
        {'selector': 'td', 'props': [('border', '0.1px solid rgba(128,128,128,0.1)')]}
    ])
    
    # st.write(styled_df) だとCanvas描画になる可能性があるため、確実にHTMLとして表示
...
with tab1:
    ...
    display_styled_table(df_all[pl_cols + plan_names])
    
    st.subheader("📋 簡易CF計算書 (月次)")
    cf_cols = ["年月", "当期純利益", "減価償却費", "簡易CF", "月返済額", "現預金増減", "借入金残高", "現預金残高", "現預金月商倍率"]
    display_styled_table(df_all[cf_cols])
...
with tab2:
    ...
    display_styled_table(df_yearly[["年度"] + pl_cols[1:] + plan_names])
    st.subheader("📊 年次簡易CF計算書")
    display_styled_table(df_yearly[["年度"] + cf_cols[1:]])
